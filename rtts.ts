...
if (!('isInteger' in Number))
    (<any>Number).isInteger = (value: any) => typeof value === 'number' && isFinite(value) && Math.floor(value) === value;
...
function checkType(val: any, ctype: string, type: string, key: string, cls: string): any {
    ...
    switch (type) {
        case 'array':
            ...
            break;
        case 'int':
            err = !(<any>Number).isInteger(val);
            if (isFloat(val)) ctype = 'float';
            break;
        case 'float':
            err = !isFloat(val);
            if ((<any>Number).isInteger(val)) ctype = 'integer';
            break;
        case 'cast_int':
